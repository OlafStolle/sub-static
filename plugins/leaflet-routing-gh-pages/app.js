...
(function() {
  ...
  jQuery(function($) {
    ...
    if (api.length === 2) {
      rUrl = 'https://' + api[1] + '/route/?coords='
      sUrl = 'https://' + api[1] + '/bbox/?bbox=';
      apiKey = api[0];
    } else {
    ...
    topo = L.tileLayer('https://opencache.statkart.no/gatekeeper/gk/gk.open_gmaps?layers=topo2&zoom={z}&x={x}&y={y}', {
      maxZoom: 16,
      attribution: '<a href="https://www.statkart.no/">Statens kartverk</a>'
    });

    var summer = L.tileLayer('https://mt3.turistforeningen.no/prod/trail_summer/{z}/{x}/{y}.png', {
      maxZoom: 16,
      attribution: '<a href="https://www.turistforeningen.no/">DNT</a>'
    });
    var winter = L.tileLayer('https://mt3.turistforeningen.no/prod/trail_winter/{z}/{x}/{y}.png', {
      maxZoom: 16,
      attribution: '<a href="https://www.turistforeningen.no/">DNT</a>'
    });
    var cabin = L.tileLayer('https://mt3.turistforeningen.no/prod/cabin/{z}/{x}/{y}.png', {
      maxZoom: 16,
      attribution: '<a href="https://www.turistforeningen.no/">DNT</a>'
    });
    ...
    $('#eta-export').on('click', function() {
      ...
      if (confirm('Eksport til ETA vil overskrive eksisterende geometri!')) {
        ...
        $.post('https://mintur.ut.no/lib/ajax/post_geom.php?api_key=' + apiKey + '&tp_id=' + id, {coords: data}, function(data) {
          if (data.error) {
            alert('Eksport feilet med feilkode ' + data.error);
          } else if (data.success) {
            window.location.href = 'https://mintur.ut.no/index.php?tp_id=' + id + '&tab=kart';
    ...
    $('#eta-import').on('click', function() {
      var id = $('#eta-id').val();
      if (!id) { alert('Ingen tp_id definert!'); return; }
      $.get('https://mintur.ut.no/lib/ajax/post_geom.php?api_key=' + apiKey + '&tp_id=' + id, function(data) {
        if (data.error) {
